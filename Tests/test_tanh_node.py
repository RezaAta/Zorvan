...
def test_tanh_node_operation():
    ...
    for x in inputs:
        ...
        assert np.isclose(out, expected), f"TanhNode returned {out} for input {x}, expected {expected}"


def test_tanh_derivative_node_operation():
    dnode = TanhDerivativeNode(name='tanh_deriv')
    values = [0, 0.5, -0.5, 0.964027580075817, -0.964027580075817]  # tanh values for known inputs
    for val in values:
        expected = 1 - (val ** 2)
        out = dnode.Operation(val)
        assert np.isclose(out, expected), f"TanhDerivativeNode returned {out} for input {val}, expected {expected}"
...
def test_tanh_and_derivative_in_graph_manual_processing():
    ...
    for _ in range(5):
        input_node.UpdateInputs(); input_node.ProcessBatch()
        tanh_node.UpdateInputs(); tanh_node.ProcessBatch()
        tanh_deriv_node.UpdateInputs(); tanh_deriv_node.ProcessBatch()
        tanh_val = tanh_node.value
        deriv_val = tanh_deriv_node.value
        assert np.isclose(deriv_val, 1 - tanh_val ** 2), f"derivative {deriv_val} != 1 - tanh^2 ({1 - tanh_val ** 2})"
