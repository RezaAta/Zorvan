...
class TestPlotViewModelInitialization:
    ...
    def test_default_initialization(self):
        ...
        # PlotViewModel no longer tracks `max_iterations`; it exposes `buffer_size`.
        assert vm.get_buffer_size() == 10000
    ...
    def test_custom_initialization(self):
        ...
        # Use buffer_size for PlotViewModel (max_iterations belongs to PlotConfigViewModel)
        vm = PlotViewModel(buffer_size=500, backend="pyqtgraph")

        assert vm.get_buffer_size() == 500
...
class TestPlotViewModelConfiguration:
    ...
    def test_set_buffer_size(self):
        """Test setting buffer size (controls stored data points)."""
        vm = PlotViewModel()
        vm.set_buffer_size(5000)

        assert vm.get_buffer_size() == 5000

    def test_set_buffer_size_minimum(self):
        """Test buffer size minimum is enforced (minimum 100)."""
        vm = PlotViewModel()
        vm.set_buffer_size(10)

        assert vm.get_buffer_size() == 100  # Minimum enforced in set_buffer_size()
    ...
    def test_set_backend(self):
        ...
        changes = []
        vm.observe_property(
            "backend_changed", lambda old, new: changes.append((old, new))
        )
...
class TestPlotViewModelNodeManagement:
    ...
    def test_add_node(self):
        ...
        changes = []
        vm.observe_property(
            "nodes_changed", lambda old, new: changes.append((old, new))
        )
...
class TestPlotViewModelDataManagement:
    ...
    def test_data_trimming_at_buffer_size(self):
        """Test data is trimmed when exceeding buffer_size."""
        vm = PlotViewModel(buffer_size=10)
        vm.add_node("node_A")
        ...
        data = vm.get_plot_data("node_A")
        assert len(data) == 10  # Trimmed to buffer_size
    ...
    def test_add_data_points_batch(self):
        ...
        vm.add_data_points({"node_A": 10.0, "node_B": 20.0}, iteration=0)
...
class TestPlotViewModelObservableProperties:
    ...
    def test_data_updated_observable(self):
        ...
        observed_values = []
        vm.observe_property(
            "data_updated", lambda old, new: observed_values.append(new)
        )
    ...
    def test_nodes_changed_observable(self):
        ...
        observed_values = []
        vm.observe_property(
            "nodes_changed", lambda old, new: observed_values.append(new)
        )
    ...
    def test_backend_changed_observable(self):
        ...
        observed_values = []
        vm.observe_property(
            "backend_changed", lambda old, new: observed_values.append(new)
        )
