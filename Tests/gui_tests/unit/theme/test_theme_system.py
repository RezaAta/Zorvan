...
def test_theme_system():
    ...
    print(
        f"    Changed node font to: {node_font['family']} {node_font['size']}pt {node_font['weight']}"
    )
    ...
    assert (
        widget_legacy.theme_viewmodel is None
    ), "Legacy mode should not have viewmodel"
    ...
    event_bus.subscribe(
        EventType.THEME_COLOR_CHANGED, lambda e: events_received.append(e)
    )
    ...
    assert (
        events_received[0].payload["color_changed"] == "border"
    ), "Event payload incorrect"
