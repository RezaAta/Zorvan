...
def test_mlp_generator_creates_graph_structure():
    ...
    # check hidden node naming or compatibility with Core MLPGraph
    assert any(n.startswith("H0N") for n in names) or (
        hasattr(g, "hiddenLayerSizes") and sum(getattr(g, "hiddenLayerSizes", [])) > 0
    )
