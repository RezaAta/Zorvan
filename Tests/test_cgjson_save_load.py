...
import tempfile
import json
from ComputationalGraphs.Core.CGJsonIO import save, load
...
def test_cgjson_save_load_numpy_attrs():
    ...
    found = False
    for n in ex.nodes:
        if type(getattr(n, '__class__', None)).__name__ == 'PiecewiseLinearNode' or n.__class__.__name__ == 'PiecewiseLinearNode':
