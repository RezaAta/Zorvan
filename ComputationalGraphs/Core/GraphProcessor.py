...
class GraphProcessor:
    ...
    def __init__(self, graph, max_workers=None, verbose=True, visual=False, auto_threading=True, chunk_size=None):
        ...
        if max_workers is None:
            self.max_workers = 1 if num_nodes < 1000 else max(1, (os.cpu_count() or 4) // 2)
        ...
        if chunk_size is None:
            self.chunk_size = max(1, num_nodes // (self.max_workers * 4) if num_nodes >= 4 else 1)
        else:
    ...
    class ExecutionController:
        ...
        def from_options(opts: Optional['GraphProcessor.ExecutionOptions']):
            ...
            return GraphProcessor.ExecutionController(step_interval_ms=opts.step_interval_ms, allow_pause=opts.allow_pause)

    def ComputeGraph(self, iterations=1, exec_options: Optional[ExecutionOptions] = None, on_iteration_complete=None, controller: Optional['GraphProcessor.ExecutionController'] = None):
        ...
        if self.max_workers <= 1:
            return self.ComputeGraphSingleThread(iterations, exec_options=exec_options, on_iteration_complete=on_iteration_complete, controller=controller_obj)
    ...
    def ComputeGraphSingleThread(self, iterations=1, exec_options: Optional[ExecutionOptions] = None, on_iteration_complete=None, controller: Optional['GraphProcessor.ExecutionController'] = None):
        # Use provided controller if given, otherwise build one from exec_options
        controller_obj = controller if controller is not None else GraphProcessor.ExecutionController.from_options(exec_options)
    ...
    def ForwardProcessing(self, iterations, starting_nodes=None, stopping_nodes=None, stop_on_nth_hit=1,
                          reactivate_nodes=None, reactivate_interval=None, exec_options: Optional[ExecutionOptions] = None, controller=None):
        ...
        stopping_nodes_set = set(stopping_nodes) if stopping_nodes is not None else set(getattr(self.graph, 'stopping_nodes', []))
        ...
        if controller is None:
            controller_obj = GraphProcessor.ExecutionController.from_options(exec_options)
        else:
        ...
        for _ in range(iterations):
            ...
            if getattr(controller_obj, 'pause_event', None) is not None:
                while controller_obj.pause_event.is_set():
                    time.sleep(0.01)
            if reactivate_interval and reactivate_nodes_set and getattr(self, '_iteration_counter', 0) > 0:
            ...
            for node in newly_processed:
                ...
                for successor in self._successor_map.get(node, []):
                    ...
                    required = self._required_predecessor_count.get(successor, self._predecessor_count.get(successor, 0))
                    if self._processed_predecessor_count[successor] >= required:
                        next_active.append(successor)
                        self._node_status[successor] = 'active'
                    

            self._active_nodes = [n for n in self._active_nodes if getattr(n, 'midCalculation', False)] + next_active
            ...
            try:
                ...
                all_processed = all(status == 'processed' for status in self._node_status.values())
            except Exception:
                all_processed = False
            ...
            if self._node_status.get(src) != 'processed':
                ...
                if src not in self._soft_processed_sources:
                    ...
                    for succ in self._successor_map.get(src, []):
                        self._processed_predecessor_count[succ] = self._processed_predecessor_count.get(succ, 0) + 1
                        # Activate successor if it now meets the required count
                        req = self._required_predecessor_count.get(succ, self._predecessor_count.get(succ, 0))
                        if self._processed_predecessor_count[succ] >= req:
                            if self._node_status.get(succ) not in ('active', 'processed'):
