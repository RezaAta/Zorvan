...
class GraphProcessor:
    def __init__(
        ...
        nodes_per_worker = (
            num_nodes / self.max_workers if self.max_workers > 0 else num_nodes
        )
    ...
    def ComputeGraph(self, iterations=1):
        ...
        if self.use_single_thread_mode:
            if self.verbose:
                worker_info = (
                    f" (auto-selected {self.max_workers} workers)"
                    if self._auto_workers
                    else ""
                )
                print(
                    f"Auto-selected single-threaded mode ({len(self.graph.nodes)} nodes{worker_info})"
                )
        ...
        if self.verbose:
            ...
            print(
                f"Starting graph computation for {iterations} iterations{worker_info}..."
            )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            for t in range(iterations):
                ...
                nodes_to_update = [
                    node for node in self.graph.nodes if not node.midCalculation
                ]
                ...
                list(
                    executor.map(
                        lambda chunk: self._process_node_chunk(chunk, "UpdateInputs"),
                        chunks,
                    )
                )
                ...
                list(
                    executor.map(
                        lambda chunk: self._process_node_chunk(chunk, "ProcessBatch"),
                        chunks,
                    )
                )
    ...
    def ForwardProcessing(self, iterations, starting_nodes=None):
        ...
        if starting_nodes is not None:
            ...
            for node in self.graph.nodes:
                if len(node.predecessors) == 0:
                    ...
                    node.__class__.__name__ == "ContainerNode"
        ...
        for iteration in range(iterations):
            if self.verbose:
                ...
                print(f"{'='*60}")
                print(
                    f"Previously computed: {[n.name for n in computed_in_previous_iteration]}"
                )
            ...
            if self.verbose:
                print(
                    f"Computed {len(computed_in_this_iteration)} nodes this iteration"
                )
