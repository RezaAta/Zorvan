...
def save_trial_plot(
    ...
    plt.plot(
        range(1, len(classic_history) + 1),
        classic_history,
        "b-",
        linewidth=2,
        label=f"Classic EA (Best: {classic_best:.6f})",
        alpha=0.7,
    )
    plt.plot(
        range(1, len(graph_history) + 1),
        graph_history,
        "g-",
        linewidth=2,
        label=f"Graph EA (Best: {graph_best:.6f})",
        alpha=0.7,
    )
    ...
    plt.title(
        f"Trial {trial_num} - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop",
        fontsize=13,
    )
...
def generate_summary(
    ...
    classic_std_curve = np.std(
        [r["history"][:max_gens] for r in classic_results], axis=0
    )
    ...
    plt.suptitle(
        f"Comparison Summary - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop",
        fontsize=14,
        y=1.02,
    )
    plt.tight_layout()
    ...
    for i, result in enumerate(classic_results):
        ax1.plot(
            ...
            alpha=0.5,
            linewidth=1.5,
            label=f"Trial {i+1} ({result['best_fitness']:.4f})",
        )
    ...
    plt.suptitle(
        f"All Trials Overlay - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop",
        fontsize=14,
        y=1.00,
    )
    plt.tight_layout()
