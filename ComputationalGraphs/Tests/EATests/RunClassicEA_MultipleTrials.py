...
def generate_summary(
    ...
    ax2.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"Median: {median_fitness:.6f}",
    )
    ...
    plt.suptitle(
        f"Summary Statistics - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop",
        fontsize=14,
        y=1.02,
    )
    plt.tight_layout()
    ...
    plt.fill_between(
        gens, min_curve, max_curve, color="lightblue", alpha=0.3, label="Min-Max Range"
    )

    plt.xlabel("Generation", fontsize=12)
    plt.ylabel("Best Fitness (Lower is Better)", fontsize=12)
    plt.title(
        f"Average Convergence - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop\n{num_trials} Trials",
        fontsize=13,
    )
    plt.grid(True, alpha=0.3)
    ...
    for result in results:
        plt.plot(
            ...
            alpha=0.6,
            linewidth=1.5,
            label=f"Trial {result['trial']} ({result['best_fitness']:.4f})",
        )

    plt.xlabel("Generation", fontsize=12)
    plt.ylabel("Best Fitness", fontsize=12)
    plt.title(
        f"All {num_trials} Trials Overlay - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop",
        fontsize=13,
    )
    plt.grid(True, alpha=0.3)
