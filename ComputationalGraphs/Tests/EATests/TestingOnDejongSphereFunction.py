"""
Test file for running the Graph-based EA on De Jong Sphere Function.
Uses the EXACT same graph structure as the GUI example.
"""
...
def run_graph_ea(pop_size=50, genome_length=5, generations=100, num_elites=1, verbose=True):
    """
    ...
    if verbose:
        print(f"Running Graph EA: {generations} generations × {pop_size} population = {total_iterations} iterations")
...
if __name__ == "__main__":
    ...
    print(f"Running Computational Graph EA: DeJong Sphere, {num_elites} elite(s), {generations} gen, {pop_size} pop")
    print("=" * 70)
    ...
    plt.plot(range(1, len(fitness_history) + 1), fitness_history, 'g-', linewidth=2, label='Best Fitness')
    plt.xlabel('Generation', fontsize=12)
    plt.ylabel('Fitness (Lower is Better)', fontsize=12)
    plt.yscale('log')  # Logarithmic scale for better visualization
    plt.title(f'Graph EA - DeJong Sphere, {num_elites} Elite, {generations} Gen, {pop_size} Pop\nBest: {best_fitness:.6f}', fontsize=12)
