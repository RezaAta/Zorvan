...
def test_toolbar_has_add_selected_to_plot_button():
    ...
    mw = MainWindow()
    assert hasattr(mw, "toolbar_add_to_plot_btn"), "Toolbar should have an 'add to plot' button"
...
def test_toolbar_does_not_have_rebuild_btn():
    ...
    assert not hasattr(mw, "rebuild_btn"), "Top toolbar should not include a 'rebuild' button"
    try:
        mw.close()
    except Exception:
        pass
