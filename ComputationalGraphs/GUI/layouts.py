...
try:
    from grandalf.graphs import Graph as GrandalfGraph, Vertex, Edge
...
def sugiyama_layout(nodes: List[Any], 
    ...
    if GRANDALF_AVAILABLE:
        return _sugiyama_grandalf(nodes, get_predecessors, node_width, node_height, 
                                   layer_spacing, node_spacing, direction)
    elif NETWORKX_AVAILABLE:
        return _sugiyama_networkx_fallback(nodes, get_predecessors, layer_spacing, 
                                            node_spacing, direction)
    else:
        # Ultimate fallback: simple grid
        return _simple_grid_layout(nodes, node_spacing)


def _sugiyama_grandalf(nodes: List[Any],
                       get_predecessors: callable,
                       node_width: float,
                       node_height: float,
                       layer_spacing: float,
        ...
        for v in component.sV:
            if v.view.xy is not None:
                ...
                if direction == 'LR':
                    ...
                    positions[v.data] = (y * layer_spacing / node_height, 
                                         x * node_spacing / node_width)
                else:  # TB
                    positions[v.data] = (x * node_spacing / node_width, 
...
def _sugiyama_networkx_fallback(nodes: List[Any],
                                 get_predecessors: callable,
                                 layer_spacing: float,
                                 node_spacing: float,
                                 direction: str) -> Dict[Any, Tuple[float, float]]:
...
def tree_layout(nodes: List[Any],
                get_predecessors: callable,
                node_spacing: float = 100,
...
def mlp_layered_layout(nodes: List[Any],
                       get_predecessors: callable,
                       node_spacing: float = 100,
    ...
    patterns = {
        ...
        'activation': [r'sigmoid', r'Sigmoid', r'relu', r'ReLU', r'tanh', r'Tanh', 
                       r'softmax', r'Softmax', r'^act', r'^Act'],
        'output': [r'^y\d+$', r'output', r'Output', r'^out', r'^Out'],
        'loss': [r'loss', r'Loss', r'error', r'Error', r'MSE', r'mse', r'CrossEntropy'],
        'gradient': [r'^dW', r'^dw', r'^grad', r'^Grad', r'^EG', r'^LRMult', r'delta', r'Delta'],
    }
    ...
    # Define layer order
    layer_order = ['input', 'buffer', 'weight', 'mult', 'add', 'activation', 
                   'output', 'loss', 'gradient', 'other']
...
def _simple_grid_layout(nodes: List[Any], spacing: float = 100) -> Dict[Any, Tuple[float, float]]:
...
class MLPLayoutEngine:
    ...
    def get_successors_by_pattern(self, node: Any, pattern: str) -> List[Any]:
        ...
        regex = re.compile(pattern)
        return [s for s in self.successors.get(node, []) 
    ...
    def get_predecessors_by_pattern(self, node: Any, pattern: str) -> List[Any]:
        ...
        regex = re.compile(pattern)
        return [p for p in self.predecessors.get(node, []) 
    ...
    def _layout_hidden_layers(self, base_x: int, base_y: int):
        ...
        for layer_id in sorted(layers.keys()):
            ...
            for j, add_node in enumerate(layer_adds):
                ...
                activations = self.get_successors_by_pattern(add_node, r'^Act_|^Sigmoid|^ReLU|^Tanh')
                if not activations:
                ...
                for act in activations:
                    ...
                    act_buffers = self.get_successors_by_pattern(act, r'^Buff_H|^Buff_Act')
                    ...
                    next_mults = self.get_successors_by_pattern(act, r'^Mul_H\d+N\d+H\d+N\d+$')
                    if not next_mults:
                        ...
                        next_mults = [m for m in all_successors if 'y' not in self.get_node_name(m)]
                    if next_mults:
                        ...
                        for nm in next_mults:
                            ...
                            if match:
                                ...
                                all_layer_mults = self.find_nodes_by_pattern(rf'^Mul_{layer_pattern}N\d+')
                        ...
                        for nm in next_mults:
                            ...
                            nm_idx = all_next_layer_mults.index(nm) if nm in all_next_layer_mults else 0
    ...
    def _layout_output_layer(self):
        ...
        if output_nodes:
            ...
            for i, output in enumerate(output_nodes):
                ...
                errors = self.get_successors_by_pattern(output, r'^Error_y|^error|Error')
                for error in errors:
                    self.place_node(error, out_base_x + 2, out_gy)
                    
                    # Label at (+1, -1) from error = bottom-right of error
                    labels = self.get_predecessors_by_pattern(error, r'^L_y|^label|Label')
                    for label in labels:
                        self.place_node(label, out_base_x + 3, out_gy - 1)
                    
                    # MSE at (+2, 0) from error
                    mse_nodes = self.get_successors_by_pattern(error, r'^MSE|^mse|Loss|loss')
    ...
    def _layout_backprop(self):
        ...
        if lr_nodes and self.positions:
            ...
            max_gy = max(self.get_grid_coords(n)[1] for n in self.placed)  # Highest Y = topmost
