...
class GraphBuilderController:
    ...
    def _is_verbose(self):
        ...
        try:
            return hasattr(self.main_window, 'verbose_check') and self.main_window.verbose_check.isChecked()
        except Exception:
            return False
    ...
    def visualize_graph_on_canvas(self, graph):
        ...
        try:
            ...
            t5 = time.time()
            if self._is_verbose():
                print(f"Node creation {t2 - t1:.3f}s, Edge creation {t3 - t2:.3f}s, Visual update {t5 - t4:.3f}s")
    ...
    def _should_skip_collision(self, node, use_gui_positions):
        ...
        return lock_enabled and (is_container or nm.startswith('dW_') or nm.startswith('dw_'))
    
    def _adjust_for_collisions(self, x, y, created_positions, min_distance, skip_collision):
                    ...
                    for pred in node.predecessors:
                        ...
                        if source_item:
                            try:
                                ...
                                print(f"Failed to create EdgeItem: {getattr(pred, 'name', pred)} -> {getattr(node, 'name', node)}")
                                raise
