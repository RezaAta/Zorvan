...
class LayoutManager:
    ...
    def snap_point(self, x: float, y: float, step: Optional[int] = None) -> Tuple[float, float]:
    ...
    def compute_positions(self, node_items: Dict[Any, Any], layout_type: str = "ann") -> Dict[Any, Tuple[float, float]]:
    ...
    def compute_ann_layout(self, nodes, scale: float = 300) -> Dict[Any, Tuple[float, float]]:
    ...
    def compute_good_layout(self, nodes, node_names: Dict[Any, str], scale: float = 300) -> Dict[Any, Tuple[float, float]]:
        ...
        for node in nodes:
            ...
            self._categorize_node(
                node, name, inputs, buffs_x, weights, muls, 
                adds, acts, buffs_h, derivatives, outputs_add, outputs_act
            )

        # Compute grid dimensions
        step_per_layer = 8
        num_hidden_layers = max(adds.keys()) + 1 if adds else 0
        _max_hidden_neurons = max(
        ...
        for node in muls:
            ...
            self._place_multiplication_node(
                node, name, pos, weights, weight_row_map, 
                weight_col_map, step_per_layer, cell_to_pixel
            )
    ...
    def _categorize_node(self, node, name, inputs, buffs_x, weights, muls, 
                         adds, acts, buffs_h, derivatives, outputs_add, outputs_act):
    ...
    def _place_multiplication_node(self, node, name, pos, weights, 
                                   weight_row_map, weight_col_map, 
                                   step_per_layer, cell_to_pixel):
...
class NodeColorizer:
    ...
    def get_node_color(cls, name: str) -> QColor:
        ...
        if (name.startswith('Label_') or name.startswith('yd') or 
            name.startswith('Error_') or (name.startswith('e') and 'E' in name)):
            return cls.COLOR_LABEL
        
        # Blue: Weights, LR, dW nodes
        if (name.startswith('W_') or name.startswith('wn') or name.startswith('wx') or 
        ...
        if name.startswith('D_') or 'derivative' in name.lower() or 'Derivative' in name:
            return cls.COLOR_DERIVATIVE
        
        # Purple: Backprop gradient nodes
        if (name.startswith('EG_') or name.startswith('LRMult_') or name.startswith('WGS_') or 
