...
class ExecutionController:
    ...
    def play(self):
        ...
        try:
            canvas_nodes = set(self.canvas.node_items.keys()) if hasattr(self.canvas, 'node_items') and self.canvas.node_items is not None else set()
        ...
        try:
            ...
            if processor_type == "forward":
                ...
                processor.ManualProcessing(
                    ...
                    computation_sequence=getattr(self.graph, 'manual_processing_sequence', None)
                )
            else:
            ...
            if self.main_window.plot_window and self.main_window.plot_window.isVisible():
    ...
    def on_step_completed(self, step):
        ...
        if self.main_window.skip_visualization:
            ...
            if not self.main_window.skip_plotting and self.main_window.plot_window and self.main_window.plot_window.isVisible():
                try:
        ...
        if self.graph_runner.processor_type in ("forward", "manual"):
            ...
            if self.main_window.dim_processed_check.isChecked() and self.graph_runner.processor_type == 'forward':
