...
class VisualizationController:
    ...
    def auto_detect_range(self):
        ...
        # Update labels
        self.main_window.min_value_label.setText(f"{self.main_window.min_value_range:.2f}")
        self.main_window.max_value_label.setText(f"{self.main_window.max_value_range:.2f}")
    ...
    def choose_min_color(self):
        ...
        if color.isValid():
            ...
            self.main_window.min_color_btn.setStyleSheet(f"background-color: {color.name()};")
            if self.main_window.colorize_enabled:
                self.canvas.update_node_visuals(
                    True,
                    self.main_window.min_value_range,
    ...
    def choose_max_color(self):
        ...
        if color.isValid():
            ...
            self.main_window.max_color_btn.setStyleSheet(f"background-color: {color.name()};")
            if self.main_window.colorize_enabled:
                self.canvas.update_node_visuals(
                    True,
                    self.main_window.min_value_range,
    ...
    def choose_node_color(self):
        ...
        color = QColorDialog.getColor(
            self.main_window.default_node_color, 
            self.main_window, 
            "Choose Node Color"
        )
        if color.isValid():
            self.main_window.default_node_color = color
            self.main_window.node_color_btn.setStyleSheet(f"background-color: {color.name()};")
    ...
    def choose_text_color(self):
        ...
        if color.isValid():
            ...
            self.main_window.text_color_btn.setStyleSheet(f"background-color: {color.name()};")
    ...
    def apply_node_colors(self):
        ...
        for node_item in self.canvas.node_items.values():
            ...
            if hasattr(node_item, 'value_label') and node_item.value_label is not None:
                node_item.value_label.setDefaultTextColor(self.main_window.default_text_color)
    ...
    def on_skip_viz_changed(self, state):
        ...
        if self.main_window.skip_visualization:
            ...
            self.main_window.speed_slider.setEnabled(not self.main_window.max_speed_btn.isChecked())
            if hasattr(self.main_window, 'speed_spin'):
                self.main_window.speed_spin.setEnabled(not self.main_window.max_speed_btn.isChecked())
