...
class GraphEdgeController:
    ...
    def on_edge_removed(self, source_node, target_node):
        ...
        """
        mw = self.main_window
        mw.status_bar.showMessage(f"Disconnected {source_node.name} → {target_node.name}")
    ...
    def _reset_forward_state_if_needed(self):
        ...
        mw = self.main_window
        try:
            if (hasattr(mw, 'graph_runner') and 
                getattr(mw, 'graph_runner') is not None and 
                hasattr(mw.graph_runner, 'graph_processor')):
    ...
    def rebuild_graph(self):
        ...
        old_starting_nodes = list(mw.graph.starting_nodes) if hasattr(mw.graph, 'starting_nodes') else []
        old_stopping_nodes = list(mw.graph.stopping_nodes) if hasattr(mw.graph, 'stopping_nodes') else []
        old_manual_sequence = (
            list(mw.graph.manual_processing_sequence) 
            if hasattr(mw.graph, 'manual_processing_sequence') and mw.graph.manual_processing_sequence 
            else None
        )
