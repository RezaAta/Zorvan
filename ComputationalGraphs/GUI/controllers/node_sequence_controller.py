...
class NodeSequenceController:
    ...
    def add_selected_to_starting_nodes(self):
        ...
        if not self.graph:
            QMessageBox.warning(
                self.main_window, "No Graph", "Please load a graph first."
            )
            return
        ...
        if (
            not hasattr(self.graph, "starting_nodes")
            or self.graph.starting_nodes is None
        ):
    ...
    def remove_from_starting_nodes(self):
        ...
        if (
            ...
            or not self.graph.starting_nodes
        ):
            return
        ...
        self.status_bar.showMessage(
            f"Removed {removed_count} node(s) from starting nodes"
        )
    ...
    def auto_detect_starting_nodes(self):
        ...
        self.update_starting_nodes_display()
        self.status_bar.showMessage(
            f"Auto-detected {len(source_nodes)} starting node(s)"
        )
    ...
    def add_selected_to_stopping_nodes(self):
        ...
        if (
            not hasattr(self.graph, "stopping_nodes")
            or self.graph.stopping_nodes is None
        ):
    ...
    def remove_from_stopping_nodes(self):
        ...
        if (
            not self.graph
            or not hasattr(self.graph, "stopping_nodes")
            or not self.graph.stopping_nodes
        ):
            return
        ...
        self.update_stopping_nodes_display()
        self.status_bar.showMessage(
            f"Removed {removed_count} node(s) from stopping nodes"
        )
    ...
    def auto_detect_stopping_nodes(self):
        ...
        if not candidates:
            QMessageBox.warning(
                self.main_window, "No Candidates", "No ContainerNode candidates found."
            )
            return
    ...
    def add_selected_to_manual_sequence(self):
        ...
        if (
            not hasattr(self.graph, "manual_processing_sequence")
            or self.graph.manual_processing_sequence is None
        ):
            self.graph.manual_processing_sequence = []
        self.graph.manual_processing_sequence.append(step_nodes)
        step_label = (
            f"Step {len(self.graph.manual_processing_sequence)-1}: "
            + ", ".join([n.name for n in step_nodes])
        )
    ...
    def add_selected_nodes_to_selected_step(self):
        ...
        self.manual_sequence_list.item(step_index).setText(
            f"Step {step_index}: " + ", ".join(labels)
        )
    ...
    def replace_selected_step_with_selected_nodes(self):
        ...
        self.status_bar.showMessage(
            f"Replaced Step {step_index} with {len(nodes_to_set)} node(s)"
        )
    ...
    def remove_from_manual_sequence(self):
        ...
        if (
            not self.graph
            or not hasattr(self.graph, "manual_processing_sequence")
            or not self.graph.manual_processing_sequence
        ):
            QMessageBox.information(
                self.main_window, "No Sequence", "Manual sequence is empty."
            )
            return
    ...
    def apply_manual_sequence_to_graph(self):
        ...
        for i in range(self.manual_sequence_list.count()):
            ...
            for nm in node_names:
                ...
                if nm in self.graph.idToNodeDictionary:
                    ...
                    # match by node name
                    matches = [
                        n for n in self.graph.nodes if getattr(n, "name", None) == nm
                    ]
                    if matches:
        ...
        try:
            ...
            QMessageBox.critical(
                self.main_window, "Apply Failed", f"Failed to set manual sequence: {e}"
            )
