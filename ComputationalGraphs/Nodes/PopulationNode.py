...
class PopulationNode(BufferNode):
    ...
    def __init__(
        ...
        if data is None and auto_generate:
            data = self.generate_population(
                size, genome_length, lower_bound, upper_bound
            )
    ...
    def generate_population(
        pop_size: int, genome_length: int, lower: float = -5.12, upper: float = 5.12
    ):
        """
    ...
    def regenerate_population(self):
        ...
        new_population = self.generate_population(
            self.bufferSize, self.genome_length, self.lower_bound, self.upper_bound
        )
