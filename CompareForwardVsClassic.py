...
backprop_forward = BackpropGraphForwardProcessing(mlp_forward, learningRate=learning_rate)
...
y_train_list = [[float(y[0])] for y in y_train]  # y_train already has nested structure [[0], [1], ...]
...
for i in range(len(X_train)):
    ...
    processor_forward.ForwardProcessing(iterations=20, starting_nodes=mlp_forward.starting_nodes)
    
    # Read output from activation node
    prediction = mlp_forward.outputLayer[0][1].value
    binary_pred = 1 if prediction > 0.5 else 0
    actual = y_train[i][0]
    print(f"Input: {X_train[i]} -> Predicted: {prediction:.4f} ({binary_pred}), Actual: {int(actual)}")
    predictions_forward.append(prediction)

mae_forward = np.mean([abs(predictions_forward[i] - y_train[i][0]) for i in range(len(y_train))])
...
for i in range(len(X_classic)):
    ...
    print(f"Input: {X_classic[i].tolist()} -> Predicted: {prediction:.4f} ({binary_pred}), Actual: {int(actual)}")
...
print(f"  Difference:         {abs(mse_history_forward[-1] - mse_history_classic[-1]):.6f}")
...
mse_close = np.allclose(mse_history_forward[-1], mse_history_classic[-1], rtol=1e-3, atol=1e-3)
...
plt.subplot(1, 2, 1)
plt.plot(range(1, epochs + 1), mse_history_forward, 
         label='Forward Processing', linewidth=2, color='#2A9D8F')
plt.plot(range(1, epochs + 1), mse_history_classic, 
         label='Classic MLP', linewidth=2, color='#E63946', linestyle='--', alpha=0.8)
plt.xlabel('Epoch')
...
plt.subplot(1, 2, 2)
mse_diff = [abs(mse_history_forward[i] - mse_history_classic[i]) 
...
plt.tight_layout()
plt.savefig('forward_vs_classic_controlled_comparison.png', dpi=300, bbox_inches='tight')
