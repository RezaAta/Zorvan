...
if __name__ == '__main__':
    ...
    graph = builder()
    print('Graph built. Steps in manual sequence:', len(getattr(graph, 'manual_processing_sequence', [])))
    proc = GraphProcessor(graph, max_workers=1, verbose=False)
    proc.ManualProcessing(iterations=3, computation_sequence=getattr(graph, 'manual_processing_sequence', None))
