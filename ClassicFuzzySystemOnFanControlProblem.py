...
def compute_fanspeed(temperature, humidity):
    ...
    """
    mu_hot      = piecewise_linear(temperature, xs=[-20, 5, 25, 45, 100], mus=[0.0, 0.0, 0.5, 1.0, 1.0])
    mu_cold     = piecewise_linear(temperature, xs=[-20, 5, 25, 45, 100], mus=[1.0, 1.0, 0.5, 0.0, 0.0])
    mu_high_hum = piecewise_linear(humidity,    xs=[0, 25, 50, 75, 100], mus=[0.0, 0.0, 0.5, 1.0, 1.0])
    mu_low_hum  = piecewise_linear(humidity,    xs=[0, 25, 50, 75, 100], mus=[1.0, 1.0, 0.5, 0.0, 0.0])
...
def plot_piecewise_curve(xs, mus, domain_min, domain_max, title="Membership Curve", xlabel="x", ylabel="mu(x)"):
    """
...
if __name__ == "__main__":
    ...
    print(f"For Temperature={temp_test}, Humidity={hum_test} → Fanspeed ≈ {fanspeed:.3f}")

    # Plot each membership curve individually
    plot_piecewise_curve(xs=[-20, 5, 25, 45, 100], mus=[0.0, 0.0, 0.5, 1.0, 1.0],
                         domain_min=-20, domain_max=100,
                         title="Hot Temperature Membership", xlabel="Temperature", ylabel="mu_hot")

    plot_piecewise_curve(xs=[-20, 5, 25, 45, 100], mus=[1.0, 1.0, 0.5, 0.0, 0.0],
                         domain_min=-20, domain_max=100,
                         title="Cold Temperature Membership", xlabel="Temperature", ylabel="mu_cold")

    plot_piecewise_curve(xs=[0, 25, 50, 75, 100], mus=[0.0, 0.0, 0.5, 1.0, 1.0],
                         domain_min=0, domain_max=100,
                         title="High Humidity Membership", xlabel="Humidity", ylabel="mu_high_hum")

    plot_piecewise_curve(xs=[0, 25, 50, 75, 100], mus=[1.0, 1.0, 0.5, 0.0, 0.0],
                         domain_min=0, domain_max=100,
                         title="Low Humidity Membership", xlabel="Humidity", ylabel="mu_low_hum")
