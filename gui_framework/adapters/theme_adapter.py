...
class ThemeAdapter:
    ...
    def sync_legacy_to_state_store(self):
        ...
        try:
            ...
            for key in self._theme_manager.defaults.keys():
                if not key.endswith('_font_family') and not key.endswith('_font_size') and not key.endswith('_font_weight'):
                    colors[key] = self._theme_manager.theme.get(key, self._theme_manager.defaults.get(key))
            ...
            for prefix in ('ui', 'node'):
                for suffix in ('_font_family', '_font_size', '_font_weight'):
                    key = f"{prefix}{suffix}"
                    fonts[key] = self._theme_manager.theme.get(key, self._theme_manager.defaults.get(key))
    ...
    def start_bidirectional_sync(self):
        ...
        self._state_store.subscribe(StateEvent.THEME_CHANGED, self._on_state_theme_changed)
        
        # Listen to EventBus theme events
        self._event_bus.subscribe(EventType.THEME_COLOR_CHANGED, self._on_theme_color_changed)
        self._event_bus.subscribe(EventType.THEME_FONT_CHANGED, self._on_theme_font_changed)
    ...
    def stop_bidirectional_sync(self):
        ...
        try:
            self._state_store.unsubscribe(StateEvent.THEME_CHANGED, self._on_state_theme_changed)
        except Exception:
            pass
        
        # Unsubscribe from EventBus
        try:
            self._event_bus.unsubscribe(EventType.THEME_COLOR_CHANGED, self._on_theme_color_changed)
            self._event_bus.unsubscribe(EventType.THEME_FONT_CHANGED, self._on_theme_font_changed)
