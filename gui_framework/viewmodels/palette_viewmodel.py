...
class PaletteViewModel(BaseViewModel):
    ...
    def initialize(self) -> None:
        ...
        # If no saved palette state or it doesn't include 'palette', expand all categories by default
        if state is None or not hasattr(state, "palette"):
            self._expanded_categories = set(self._categories.keys())
        else:
            try:
                palette_state = state.palette
                self._search_text = getattr(palette_state, "search_text", "")
                expanded = list(getattr(palette_state, "expanded_categories", []))
                # If stored expanded list is empty, default to expanding all categories
                if expanded:
                    self._expanded_categories = set(expanded)
                else:
                    self._expanded_categories = set(self._categories.keys())
            except Exception:
                self._expanded_categories = set(self._categories.keys())
    ...
    def expand_category(self, category_name: str) -> None:
        """Expand a category."""
        if (
            category_name in self._categories
            and category_name not in self._expanded_categories
        ):
            self._expanded_categories.add(category_name)
            self._save_state()
            self._notify_observers("expanded_categories")
        ...
        if not self._search_text:
            ...
            self._filtered_nodes = [
                ...
                if (
                    ...
                    or search_lower in node.node_type.lower()
                    or search_lower in node.description.lower()
                    or search_lower in node.category.lower()
                )
            ]
