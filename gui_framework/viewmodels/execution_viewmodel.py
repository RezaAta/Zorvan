...
class ExecutionViewModel(BaseViewModel):
    ...
    def __init__(self, max_steps: int = 100, speed_ms: int = 100, **kwargs):
        """Initialize ExecutionViewModel with optional initial values.

        Args:
            max_steps: Initial maximum steps for execution (default: 100)
            speed_ms: Initial execution speed in milliseconds (default: 100)
            **kwargs: Forward compatibility for additional init args
        """
        # Initialize base view-model
        super().__init__()

        # Set initial observable properties
    ...
    def initialize(self):
        ...
        self._event_bus.subscribe(
            EventType.EXECUTION_STEP_COMPLETE, self._on_step_complete
        )
    ...
    def play(self) -> bool:
        ...
        self._event_bus.publish(
            Event(
                type=EventType.EXECUTION_STARTED, payload={"max_steps": self.max_steps}
            )
        )
    ...
    def resume(self) -> bool:
        ...
        if self.status == ExecutionStatus.COMPLETED:
            ...
            self._event_bus.publish(
                Event(type=EventType.EXECUTION_STARTED, payload={"resume": True})
            )
    ...
    def set_speed(self, speed_ms: int):
        ...
        self._event_bus.publish(
            Event(
                type=EventType.EXECUTION_SPEED_CHANGED, payload={"speed_ms": speed_ms}
            )
        )
    ...
    def toggle_max_speed(self):
        ...
        if self.is_max_speed:
            ...
            self.set_speed(self._previous_speed_ms)
        else:
            # Store current speed before going to max
            if self.speed_ms > 0:
                self._previous_speed_ms = self.speed_ms
    ...
    def _on_step_complete(self, event: Event):
        ...
        if step is not None:
            ...
            if self.current_step >= self.max_steps:
                ...
                self._event_bus.publish(
                    Event(type=EventType.EXECUTION_STOPPED, payload={"completed": True})
                )
