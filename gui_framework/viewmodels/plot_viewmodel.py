...
class PlotViewModel(BaseViewModel):
    ...
    def __init__(self, max_iterations: int = 100, backend: str = 'matplotlib'):
        ...
        self._plot_data: Dict[str, List[PlotDataPoint]] = {}  # node_name -> [data points]
    ...
    def set_max_iterations(self, max_iterations: int):
        """Set maximum iterations to track.
        
        Args:
            max_iterations: Maximum iterations (minimum 10)
        """
        self._max_iterations = max(10, max_iterations)
        print(f"[PlotViewModel] Max iterations set to {self._max_iterations}")
    ...
    def add_data_point(self, node_name: str, value: float, iteration: Optional[int] = None):
        ...
        # Trim data if exceeds max iterations
        if len(self._plot_data[node_name]) > self._max_iterations:
            self._plot_data[node_name] = self._plot_data[node_name][-self._max_iterations:]
