...
X_transposed = X.T.tolist()  # [[x1_sample1, x1_sample2, ...], [x2_sample1, x2_sample2, ...]]
...
default_mse_per_epoch = np.array(default_mse_history).reshape(epochs, len(X)).mean(axis=1)
...
print(f"Buffer size: {len(sample_buffer)}, Expected: {networkLength + total_iterations}")
...
print(f"{'Default Graph (BufferNodes)':<40} {default_per_iter:>10.4f} ms  {default_per_iter/classic_per_iter:>10.2f}x")
print(f"{'Forward Processing (No Buffers)':<40} {forward_per_iter:>10.4f} ms  {forward_per_iter/classic_per_iter:>10.2f}x")
...
if forward_per_iter < default_per_iter:
    ...
    print(f"   Overhead eliminated: {default_per_iter - forward_per_iter:.4f}ms per iteration")
else:
    slowdown = forward_per_iter / default_per_iter
    print(f"Forward Processing is {slowdown:.2f}x SLOWER than Default Graph")
    print(f"   Additional overhead: {forward_per_iter - default_per_iter:.4f}ms per iteration")
...
plt.subplot(1, 2, 1)
plt.plot(range(1, epochs+1), classic_mse_history, 'b-', label='Classic (NumPy)', linewidth=2)
plt.plot(range(1, epochs+1), default_mse_per_epoch, 'r-', label='Default Graph (BufferNodes)', linewidth=2)
plt.plot(range(1, epochs+1), forward_mse_history, 'g-', label='Forward Processing', linewidth=2)
plt.xlabel('Epoch')
plt.ylabel('MSE')
...
print("=" * 80)
print(f"{'Classic (NumPy)':<30} {classic_mse_history[-1]:<15.6f} {mae_classic:<15.6f} {classic_per_iter:<15.4f}")
print(f"{'Default Graph (BufferNodes)':<30} {default_mse_per_epoch[-1]:<15.6f} {mae_default:<15.6f} {default_per_iter:<15.4f}")
print(f"{'Forward Processing':<30} {forward_mse_history[-1]:<15.6f} {mae_forward:<15.6f} {forward_per_iter:<15.4f}")
print("=" * 80)
