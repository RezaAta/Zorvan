...
class ClassicMLP:
    def __init__(self, input_size, hidden_sizes, output_size, learning_rate=0.01):
        ...
        # Initialize weights
        self.weights_input_hidden = np.ones(
            (input_size, hidden_sizes[0])
        )  # Input -> Hidden Layer 1
        self.weights_hidden_hidden = np.ones(
            (hidden_sizes[0], hidden_sizes[1])
        )  # Hidden Layer 1 -> Hidden Layer 2
        self.weights_hidden_output = np.ones(
    ...
    def backward(self, X, y):
        ...
        self.weights_hidden_output += self.learning_rate * np.dot(
            self.hidden2_output.T, delta_output
        )
        self.weights_hidden_hidden += self.learning_rate * np.dot(
            self.hidden1_output.T, delta_hidden2
        )
