...
class MLPGraphForwardProcessing(Graph):
    def __init__(
        ...
        Args:
            ...
            hiddenActivationFunctions: Single class or list of classes for hidden layers
            hiddenLayerSizes: List of neurons per hidden layer.
                Default: all same as numInputs.
        ...
        self.errorBuffers = []
        self.mseNodes = []
        # optional initializer object for weights/biases
    ...
    def BuildMLP(self):
        ...
        # These multiplication nodes should be the starting nodes, not inputs/weights.
        ...
        # Labels are NOT starting nodes; they are data holders read by Error nodes.
    ...
    def LoadData(self, X_data, y_data):
        ...
        if len(X_data[0]) != self.numInputs:
            raise ValueError(
                (
                    f"X_data feature count {len(X_data[0])} "
                    f"must match numInputs {self.numInputs}"
                )
            )
        if len(y_data[0]) != self.numOutputs:
            raise ValueError(
                (
                    f"y_data feature count {len(y_data[0])} "
                    f"must match numOutputs {self.numOutputs}"
                )
            )
    ...
    def PrepareForForwardProcessing(self, processor):
        """
        Prepare the MLP graph for forward processing by marking source nodes
        as processed.

        This solves the "branch processing issue" where starting nodes
        (first layer mult nodes) need to access source nodes (inputs, weights)
        that haven't been "processed" yet but already have values.

        Marks as processed:
        - Input DataStreamNodes (have data loaded) - source nodes.
        - Label DataStreamNodes (have data loaded) - source nodes.
        - Weight ContainerNodes (have initialized values) - may have dW predecessors.

        Call this AFTER LoadData() and AFTER adding backprop (if training),
        BEFORE starting forward processing.
        ...
        Example usage:
            ...
            mlp.PrepareForForwardProcessing(processor)
                # Mark sources and weights as processed
        ...
        try:
            ...
            # differently.
            # They will not call successors during the newly-processed iteration.
            ...
            if (
                ...
                if processor.verbose:
                    print(
                        (
                            f"Copied {len(self.stopping_nodes)} "
                            f"stopping_nodes into processor.graph"
                        )
                    )
        except Exception:
    ...
    def _CreateInputLayer(self):
        """Create input layer with DataStreamNodes.

        Streams through data automatically.
        """
        from zorvan.Nodes.DataStreamNode import DataStreamNode

        # initialDelay=0 and streamDelay=0 for ForwardProcessing.
        # No synchronization is needed in this mode.
        self.inputLayer = [
    ...
    def _CreateHiddenLayers(self):
        ...
        for layerNum, numNeurons in enumerate(self.hiddenLayerSizes):
            ...
            for i in range(numNeurons):
                ...
                # Activation node (support per-layer activation classes)
    ...
    def _CreateWeightLayers(self):  # noqa: C901
        ...
        # Biases: one per hidden neuron and output neuron
    ...
    def _CreateLabelLayer(self):
        """Create label layer with DataStreamNodes.

        Streams through labels automatically.
        """
        from zorvan.Nodes.DataStreamNode import DataStreamNode
    ...
    def CreateErrorBuffers(self, bufferSize, allowNone=True, mse_buffer_size=None):  # noqa: C901
        """
        ...
        Returns:
            List[BufferNode]: The created buffer nodes.
                Also stored in `self.errorBuffers`.
        """
        ...
        try:
            ...
            self.mseNodes = []
        # Infer mse_buffer_size if not provided.
        # Forward mode stores data row-per-sample.
        ...
        for i, err_node in enumerate(self.errorLayer):
            ...
            self.AddNode(buf)
            # Create an MS node for this error (mean of squared errors over buffer)
    ...
    def GetPassLength(self):
        """
        Get the length of one complete training pass.

        Alias for GetRequiredIterationsWithBackprop. This is the
        reactivation interval for input/label nodes in cyclic training.
